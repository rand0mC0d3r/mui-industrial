...
function IndustrialProvider(_a) {
    ...
    var handleKeyboardAnnouncement = function (_a) {
        var id = _a.id, label = _a.label, ascii = _a.ascii, char = _a.char, shiftKey = _a.shiftKey, ctrlKey = _a.ctrlKey, altKey = _a.altKey, insensitive = _a.insensitive, onTrigger = _a.onTrigger;
        setShortcuts(function (shortcuts) {
            var findError = shortcuts.find(function (shortcut) { return shortcut.id === id; });
            if (findError) {
                console.error("".concat(packageName, ": \u274C Same shortcut already registered with id: [").concat(id, "]"));
            }
            return __spreadArray(__spreadArray([], shortcuts.filter(function (shortcut) { return shortcut.id !== id; }), true), [
                {
                    id: id,
                    char: char,
                    open: false,
    ...
    var handleKeyboardTriggerUpdate = function (_a) {
        var id = _a.id, onTrigger = _a.onTrigger;
        setShortcuts(function (shortcuts) { return __spreadArray([], shortcuts.map(function (shortcut) { return shortcut.id !== id ? shortcut : __assign(__assign({}, shortcut), { onTrigger: onTrigger }); }), true); });
    };
    var handleCallKeyboard = function (_a) {
        var id = _a.id;
        var findShortcut = shortcuts.find(function (shortcut) { return shortcut.id === id; });
        if (findShortcut !== undefined && (findShortcut === null || findShortcut === void 0 ? void 0 : findShortcut.onTrigger)) {
            findShortcut === null || findShortcut === void 0 ? void 0 : findShortcut.onTrigger(findShortcut.open);
        }
        setShortcuts(function (shortcuts) { return __spreadArray([], shortcuts.map(function (shortcut) { return shortcut.id !== id ? shortcut : __assign(__assign({}, shortcut), { open: !shortcut.open }); }), true); });
    };
    ...
    useEffect(function () {
        if (settings.debug) {
            ...
            console.table(__assign({}, settings));
            console.log('%cStatus', 'color: #2196f3');
            console.table(__assign({}, status));
            console.log('%cSnackbar', 'color: #f44336');
            console.table(__assign({}, snackbar));
            console.log('%cShortcuts', 'color: #ff9800');
            console.table(__assign({}, shortcuts));
        }
    ...
    return _jsx(DataContext.Provider, __assign({ value: {
            ...
            // keyboard
            shortcuts: shortcuts,
            handleKeyboardAnnouncement: handleKeyboardAnnouncement,
            handleKeyboardTriggerUpdate: handleKeyboardTriggerUpdate,
