...
var IndustrialProvider = function (_a) {
    ...
    var handleKeyboardAnnouncement = function (_a) {
        var id = _a.id, label = _a.label, ascii = _a.ascii, char = _a.char, shiftKey = _a.shiftKey, ctrlKey = _a.ctrlKey, altKey = _a.altKey, metaKey = _a.metaKey, onTrigger = _a.onTrigger, insensitive = _a.insensitive;
        // console.log('registed keyboard', id);
        var findShortcut = shortcuts.find(function (shortcut) { return shortcut.id === id; });
        var evaluate = !!findShortcut
            && JSON.stringify(findShortcut) !== JSON.stringify({ id: id, label: label, ascii: ascii, char: char, shiftKey: shiftKey, ctrlKey: ctrlKey, altKey: altKey, metaKey: metaKey, onTrigger: onTrigger, insensitive: insensitive });
        if (!evaluate) {
            console.log('updating shortcuts', id);
            setShortcuts(function (prevShortcuts) {
                return __spreadArray(__spreadArray([], prevShortcuts.filter(function (shortcut) { return shortcut.id !== id; }), true), [
                    {
                        id: id,
                        char: char,
                        label: label,
    ...
    var handleUpdateKeyboard = function (_a) {
        var id = _a.id, onTrigger = _a.onTrigger;
        setShortcuts(function (prevShortcuts) { return prevShortcuts.map(function (p) { return (p.id === id ? __assign(__assign({}, p), { onTrigger: onTrigger }) : p); }); });
    };
    var handleCallKeyboard = function (_a) {
        ...
        if (!!findShortcut && (findShortcut === null || findShortcut === void 0 ? void 0 : findShortcut.onTrigger)) {
            ...
            findShortcut === null || findShortcut === void 0 ? void 0 : findShortcut.onTrigger(new Date().toTimeString());
        }
    };
