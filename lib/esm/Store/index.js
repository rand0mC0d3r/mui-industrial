...
var IndustrialProvider = function (_a) {
    ...
    function generateSignature(id, label, ascii, char) {
        return "".concat(id, "-").concat(label, "-").concat(ascii, "-").concat(char);
    }
    // KEYBOARD SHORTCUTS
    var handleKeyboardAnnouncement = function (_a) {
        ...
        setShortcuts(function (prevShortcuts) {
            return __spreadArray(__spreadArray([], prevShortcuts
                .filter(function (p) { return p.id !== id; }), true), [
                {
                    id: id,
                    char: char,
                    label: label,
    ...
    var handleCallKeyboard = function (_a) {
        ...
        setShortcuts(function (prevShortcuts) {
            return __spreadArray(__spreadArray([], prevShortcuts
                .filter(function (p) { return p.id !== id; }), true), [
                __assign(__assign({}, findShortcut), { lastCall: new Date().getTime() }),
            ], false);
        });
        // new Date().getTime()
    };
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    var handleKeyboardDestroy = function (_a) {
        var id = _a.id;
        if (shortcuts.some(function (shortcut) { return shortcut.id === id; })) {
            setShortcuts(function (prevShortcuts) {
                if (settings.debug) {
                    console.log('[store] 💀 Destroyed keyboard', id, __spreadArray([], prevShortcuts.filter(function (p) { return p.id !== id; }), true));
                }
                return __spreadArray([], prevShortcuts.filter(function (p) { return p.id !== id; }), true);
            });
        }
    };
    //////////////////////////
    ...
    return _jsx(DataContext.Provider, __assign({ value: {
            ...
            handleKeyboardDestroy: handleKeyboardDestroy,
