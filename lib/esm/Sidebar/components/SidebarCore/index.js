...
export var StatusCore = forwardRef(function (props, ref) {
    ...
    var renderAction = _jsx(Tooltip, __assign({ title: tooltip, arrow: true, placement: 'right' }, { children: _jsx(Button, __assign({ ref: ref, onClick: function () { return updateSidebarIndex(id); }, variant: "text", style: { minWidth: 'unset' } }, { children: cloneElement(icon, {
                style: { fontSize: '28px' },
                color: id === sidebarIndex ? 'primary' : 'action',
            }) })) }), id);
    return _jsx(_Fragment, { children: (sidebarObject !== null && !!id && sidebarObject.visible && children)
            && _jsxs(_Fragment, { children: [(!additional && elementFoundActions) && createPortal(renderAction, elementFoundActions), (additional && elementFoundAdditional) && createPortal(renderAction, elementFoundAdditional), elementFoundPanel && sidebarIndex === id && createPortal((sidebarObject.visible && children) && children, elementFoundPanel)] }) });
});
